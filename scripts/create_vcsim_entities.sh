...
sudo pkill vcsim
sudo /home/nutanix/go/bin/vcsim -l $ip:8989 -username $VCSIM_USERNAME -password $VCSIM_PASSWORD &
...
do
  ...
  do
    ...
    govc cluster.add -cluster=$cluster_name -hostname $host_name -username $VCSIM_USERNAME -password $VCSIM_PASSWORD -k=true
    datastore_name="LocalDS_$host_name"
    echo -e "$pline Creating $datastore_name for $host_name $pline"
    govc datastore.create -type local -name $datastore_name -path /var/local $host_name
    echo -e "$pline Adding host $host_name to DVS0 $pline"
    govc dvs.add -dvs=/DC0/network/DVS0 -k=true $host_name
    for ((vm=1; vm<=$3; vm++))
    do
      vm_name="${host_name}_VM_${vm}"
      echo -e "$pline Creating $vm_name $pline"
      govc vm.create -ds $datastore_name -pool=/DC0/host/$cluster_name/Resources -host $host_name -net="VM Network" -on=false $vm_name
      vm_ip=$(printf "%d.%d.%d.%d\n" "$((RANDOM % 256 ))" "$((RANDOM % 256 ))" "$((RANDOM % 256 ))" "$((RANDOM % 256 ))")
      vm_mac=$(printf "00:60:2f$(od -txC -An -N3 /dev/random|tr \  :)")
      govc vm.customize -vm $vm_name -mac $vm_mac -ip $vm_ip -netmask 255.255.255.0
